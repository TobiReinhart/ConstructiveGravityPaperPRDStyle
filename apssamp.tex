...
\iffalse
\begin{definition}
A metric spacetime $(M, g)$ is called spatially homogeneous and isotropic if there exists a diffeomorphism $\phi \colon \mathbb R \times \Sigma \rightarrow M$, with induced embeddings $\phi_t \colon \Sigma \ni p \mapsto \phi(t, p)$, such that for all $t \in \mathbb R$ the metric 3-manifold $(\Sigma, (\phi_\lambda)^\ast g)$ is homogeneous and isotrop according to the definitions in \cite{Wald}.
\end{definition}
We will restrict our attention to the case of \textbf{\textit{flat}} spatial slices $\Sigma$. Note that this definition implies the existence of a \textbf{\textit{time}} vector field $\partial_t$ via the pushforward of the canonical vector field $\partial_1$ along $\phi$ \cite{2004math.ph..11032G}. For such a spacetime, there always exists coordinates such that
\begin{equation}
    g = \mathrm dt \otimes \mathrm dt - a(t)^2 (\mathrm dx \otimes \mathrm dx + \mathrm dy \otimes \mathrm dy + \mathrm dz \otimes \mathrm dz).
\end{equation}
The \textbf{\textit{scale factor}} $a$ is in fact a scalar density of weight $\frac{1}{3}$, since $\sqrt{-\operatorname{det} g} = a^3$.
\fi
