...
In chapter \ref{chapter2} we deduce perturbative equivalents to the two fundamental requirements that we phrased in chapter \ref{chapter1}. On top of that, we concern ourselves with the computation of power series solutions to the PDE that we derived from $(A1)$. This allows us to extract viable information regarding the number of independent curvature invariants any specific tensorial theory of gravity admits, recovering, in particular, the known number of $14$ curvature invariants for the case of spacetime being endowed with a metric structure. 
...
Proofing formal integrability of PDEs is notoriously difficult. There exists however the related notion of \textit{\textbf{involutive}} PDEs that compromises formal integrability, i.e., constitutes a stronger condition, whilst at the same time is much easier to verify (cf. \cite{seiler2009involution}, \cite{seiler1994analysis}).  
\begin{theorem}\label{Invol}
PDE (\ref{DiffeoEqn}) is involutive and thus, in particular, formally integrable.
\end{theorem}
\begin{proof}
We only sketch why any potential integrability condition of (\ref{DiffeoEqn}) is necessarily linearly dependent on the equations already contained in  (\ref{DiffeoEqn}) and thus add no further information to the system.
...
\end{align}
where $t := \pi_{\mathbb{R}} \circ \phi^{-1}$ and moreover,
for all $\lambda \in \mathbb{R}$, the metric $3$-manifold $(\Sigma, \gamma_{\lambda}:= \phi_{\lambda}^{\ast} \gamma)$ is euclidean.
\iffalse
For a definition of such spacetimes and its properties we refer the reader to the elaboration in \cite{Wald}.
...
\end{enumerate}
\fi
Consequently, $\gamma_{\lambda}$ is in one-to-one correspondence with its induced volume form. We define:
\iffalse
...
\end{equation}
We call $a(\lambda)$ the \textbf{\textit{scale factor}}.

Because all information about the cosmological metric spacetime is encoded in $\partial_t$ and $a$, we construct the gravitational dynamics over the \textbf{\textit{cosmological bundle}} $F_C = TM \oplus_M Vol^{\frac{1}{3}}M$ of vectors and densities of weight $\frac{1}{3}$. The vertical coefficients of vector fields are
\begin{equation}
...
\begin{equation}
    S = \frac{1}{2\kappa} \int \sqrt{-\operatorname{det} g}\left\lbrack R - 2 \Lambda\right\rbrack \mathrm d^4x
\end{equation}
